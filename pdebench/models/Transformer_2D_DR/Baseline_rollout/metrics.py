...
def metrics(
    ...
    with torch.no_grad():
        for itot, (x_in, y_tgt, _grid) in enumerate(val_loader):
            ...
            x_in = x_in.to(device, non_blocking=True)           # [B,T,C,H,W]
            y_tgt = y_tgt.to(device, non_blocking=True)

            if rollout_steps == 1:

                x_win = x_in.permute(1, 0, 2, 3, 4)              # (T,B,C,H,W)
                pred = model(x_win)                             # (B,C,H,W)

                # reshape to [B, H, W, 1, C] for metric_func
                pred_r = pred .permute(0, 2, 3, 1).unsqueeze(-2)
                y_r = y_tgt.permute(0, 2, 3, 1).unsqueeze(-2)

            else:
                # initial_step
                T_ctx = x_in.size(1)
                window = x_in.clone()                            # [B,T,C,H,W]
                preds = []

                for _ in range(rollout_steps):
                    out = model(window.permute(1, 0, 2, 3, 4))   # (B,C,H,W)
                    preds.append(out)
                    # slide the window: drop oldest, append newest
                    window = torch.cat(
                        (window[:, 1:], out.unsqueeze(1)), dim=1)

                pred_stack = torch.stack(preds, dim=1)           # [B,R,C,H,W]

                # reshape to [B, H, W, R, C] for metric_func
                pred_r = pred_stack.permute(0, 3, 4, 1, 2).contiguous()
                y_r = y_tgt     .permute(0, 3, 4, 1, 2).contiguous()
