...
from utils_aux import ReactionDiffusionAuxDataset
from transformer_aux_rd import ViT2dAux
from metrics_aux import metrics, LpLoss, FftLpLoss  # same helpers
...
def run_training(cfg: DictConfig):
    ...
    train_ds = ReactionDiffusionAuxDataset(
    ...
    val_ds = ReactionDiffusionAuxDataset(
        primary_filename="2D_diff-react_test_all",
        auxiliary_filename="2D_diff-react_test_diff",
        saved_folder=cfg.base_path,
        initial_step=cfg.initial_step,
    ...
    wandb.init(project="2D_RD_transformer_aux",
    ...
    if not cfg.if_training:
        ...
        with open(f"{cfg.model_name}_ds{'_'.join(map(str, cfg.train_subsample))}_eval_metrics.pkl", "wb") as f:
...
# Hydra entry‑point
@hydra.main(version_base=None, config_path=".", config_name="config_transformer_aux_rd")
