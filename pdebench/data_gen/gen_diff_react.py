...
def simulator(config, i):
    ...
    print(f'----simulating using {config.sim.sim_type}-----')
...
def main(config: DictConfig):
    ...
    output_path: Path = work_path  / config.output_path
    # output_path.mkdir(output_path, exist_ok=True, parents=True)
    # output_path = Path('../data')
    config.output_path = output_path.with_suffix(".h5")
    


    num_samples_init = 0
    num_samples_final = config.nsample
    ...
    if config.upload:
        dataverse_upload(
            ...
            dataverse_url=os.getenv("DATAVERSE_URL", "XXXX"),
