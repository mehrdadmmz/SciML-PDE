...
def plot_trajectory(data, t, dim, channel, config, output_dir):
    """
    Plots the entire trajectory over time instead of a single frame.
    
    Parameters:
        data (numpy array): The simulation data with shape (timesteps, x, y, channels).
        t (numpy array): Time values corresponding to each frame.
        dim (int): Dimension of the dataset (1D or 2D).
        channel (int): The index of the variable to plot.
        config (DictConfig): Configuration parameters.
        output_dir (str): Directory where frames and animation are saved.
    """
    ...
    for i in range(num_timesteps):
        ...
        plt.figure()
        plt.title(f"$t={t[i]:.2f}$")  # Ensure correct time labeling
        
        if dim == 1:
            with h5py.File(config.data_path, "r") as h5_file:
                x = np.array(h5_file["grid"]["x"], dtype="f")
            plt.plot(x.squeeze(), data[i, ..., channel])
            plt.xlabel("$x$")
        else:
            plt.imshow(
                data[i, ..., channel].transpose(),
                aspect="auto",
                origin="lower",
                extent=[
                    config.sim.x_left,
        ...
        images.append(imageio.imread(frame_path))  # Store frame for GIF

    # Save as an animated GIF
    gif_path = output_dir / config.name + "_" + postfix + "trajectory.gif"
    imageio.mimsave(gif_path, images, duration=0.1)  # Adjust speed as needed

    print(f"Trajectory animation saved at: {gif_path}")
