...
def build_vmae(params):
    ...
    if params.model_pretrained:
        checkpoint = torch.load(
            params.model_pretrained, map_location="cpu"
        )
        if "model" in checkpoint.keys():
            ...
            unload_keys = [k for k in new_state_dict.keys()
                           if k not in pretrained_dict]
            if len(unload_keys) > 0:
                import warnings
                warnings.warn(
                    "Warning: missing keys during restoring checkpoint: %s" % (str(unload_keys)))
...
class PretrainVisionTransformer(nn.Module):
    ...
    def forward(self, x, mask=None, return_feature=False):
        ...
        # https://github.com/PolymathicAI/multiple_physics_pretraining/blob/main/models/avit.py#L104
        with torch.no_grad():
        ...
        if mask is not None:
            ...
            if mask.sum() == 0:
                ...
                # TODO TBCHW: All state labels in the batch should be identical
                x = x * data_std + data_mean
            # x = x * data_std + data_mean # TODO TBCHW: All state labels in the batch should be identical
            return x
        else:
            if return_feature:
