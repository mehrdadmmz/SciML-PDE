...
# ────────────────────────────────────────────────────────────────
#  Low-level metric kernel (unchanged)
# ────────────────────────────────────────────────────────────────
def metric_func(
    pred,
    target,
    ...
    and three-band Fourier RMSE for 1-, 2- or 3-D fields.

    INPUT SHAPES (before permutation)
        pred, target : [B, X, Y(, Z), T, C]
                       A time dimension of length 1 (T missing) is accepted.
    """
    pred, target = pred.to(device), target.to(device)

    # add dummy time dim if needed (4-D input from single-step models)
    if pred.ndim == 4:
...
# ────────────────────────────────────────────────────────────────
#  High-level metrics wrapper (Transformer + rollout supported)
# ────────────────────────────────────────────────────────────────
def metrics(
    val_loader,
    model,
    ...
    val_type: str = "single",    # "single" or "rollout"
    rollout_test: int = 1,       # R steps predicted in rollout mode
    initial_step: int | None = None,
):
    """
    Aggregate metrics over `val_loader`.

    • mode       : "Transformer" (ViT-based) | "FNO" (kept from original code)
    • val_type   : "single"  → evaluate one-step predictions (train / std. val)
                   "rollout" → autoregressive evaluation over `rollout_test` steps
    """
    if mode == "Transformer":
        with torch.no_grad():
            for itot, batch in enumerate(val_loader):
                # ── SINGLE-STEP ────────────────────────────────
                if val_type == "single":
                    # y_tgt: [B,C,H,W]
                    x_in, y_tgt = batch
                ...
                # ── ROLLOUT ───────────────────────────────────
                elif val_type == "rollout":
                    # batch may be either (B,R,C,H,W) or (B,C,H,W) when R==1
                    x_seq, y_seq = batch
                    x_seq = x_seq.to(device, non_blocking=True)
                    y_seq = y_seq.to(device, non_blocking=True)

                    # restore time dim if rollout_test == 1
                    if y_seq.dim() == 4:            # [B,C,H,W]
                        y_seq = y_seq.unsqueeze(1)  # → [B,1,C,H,W]

                    preds = []
                    xx = x_seq                     # [B,T,C,H,W]
                    for _ in range(rollout_test):
                        p = model(xx)              # (B,C,H,W)
                        preds.append(p.unsqueeze(1))
                        xx = torch.cat([xx[:, 1:], p.unsqueeze(1)], dim=1)

                    pred = torch.cat(preds, dim=1)  # [B,R,C,H,W]
                    pred_last = pred[:, -1]       # (B,C,H,W)
                    target_last = y_seq[:, -1]      # (B,C,H,W)

                    pred_r = pred_last .permute(0, 2, 3, 1).unsqueeze(-2)
                    y_tgt_r = target_last.permute(0, 2, 3, 1).unsqueeze(-2)
    ...
    # ────────────────────────────
    #  Original FNO handling
    # ────────────────────────────
    elif mode == "FNO":
        # (kept unchanged from the supplied FNO implementation)
        with torch.no_grad():
            itot = 0
            for itot, (xx, yy, grid) in enumerate(val_loader):
                ...
                if val_type == "rollout":
                    ...
                    for _ in range(rollout_test):
                        pred = model(xx, grid)
                        xx_pred = torch.cat((xx_pred, pred), dim=-2)
                        xx = torch.cat((xx[..., 1:, :], pred), dim=-2)
                else:
                    pred = model(xx, grid)
                    yy_last = yy

                (_err_RMSE, _err_nRMSE, _err_CSV, _err_Max, _err_BD, _err_F) = metric_func(
                    pred,
                    yy_last,
                    if_mean=True,
                    Lx=Lx,
...
class LpLoss:
    ...
    def __call__(self, x, y, eps=1e-20):
        ...
        norm = eps + torch.norm(y.view(num_examples, -1), self.p, dim=1)
...
class FftLpLoss:
    ...
    def __call__(self, x, y, flow=None, fhigh=None, eps=1e-20):
        ...
        flow = 0 if flow is None else flow
        fhigh = np.max(xf.shape[1:]) if fhigh is None else fhigh

        slicer = (slice(None),) + (slice(flow, fhigh),) * (len(x.shape) - 1)
        xf = xf[slicer]
        yf = yf[slicer]

        diff = xf - yf.reshape_as(xf)
        diff = torch.norm(diff.reshape(num_examples, -1), self.p, dim=1)
        norm = eps + torch.norm(yf.reshape(num_examples, -1), self.p, dim=1)
        if self.reduction == "mean":
            return torch.mean(diff / norm)
        if self.reduction == "sum":
            return torch.sum(diff / norm)
...
class FftMseLoss:
    ...
    def __call__(self, x, y, flow=None, fhigh=None):
        ...
        slicer = (slice(None),) + (slice(flow, fhigh),) * (len(x.shape) - 2)
        xf = xf[slicer]
        yf = yf[slicer]

        diff = (xf - yf).reshape(num_examples, -1).abs() ** 2
        if self.reduction == "mean":
...
# inverse_metrics helper kept unchanged (omitted for brevity, same as original)
