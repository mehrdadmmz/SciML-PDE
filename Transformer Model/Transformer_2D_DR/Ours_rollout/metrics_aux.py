    ...
    *,
    ...
    with torch.no_grad():
        ...
        for batch in val_loader:
            # -------------------- move + reshape inputs --------------------
            xx, yy, xx_aux, yy_aux, grid, grid_aux = batch
            xx = xx.to(device, non_blocking=True)          # [B,T,C,H,W]
            # [B,R,C,H,W] or [B,C,H,W]
            yy = yy.to(device, non_blocking=True)
            xx_aux = xx_aux.to(device, non_blocking=True)      # [B,N,T,C,H,W]
            grid = grid.to(device, non_blocking=True)

            # (T,B,C,H,W) layout expected by ViT backbone
            xx_roll = xx.permute(1, 0, 2, 3, 4).contiguous()     # >>> CHANGED

            # flatten auxiliary samples   (T,B*N,C,H,W)
            B, N_aux, T, C, H, W = xx_aux.shape
            xx_aux_flat = xx_aux.reshape(B * N_aux, T, C, H, W)
            xx_aux_flat = xx_aux_flat.permute(1, 0, 2, 3, 4).contiguous()

            # ----------------------------------------------------------------
            # autoregressive rollout for R steps
            # ----------------------------------------------------------------
            for _ in range(rollout_steps):                       # >>> CHANGED
                pred_primary, _ = model(xx_roll, None,
                                        xx_aux_flat, None)      # one-step ahead
                # shift context window: drop first frame, append prediction
                xx_roll = torch.cat(
                    (xx_roll[1:], pred_primary.unsqueeze(0)), dim=0
                )

            # `pred_primary` is now the R-th prediction ----------------------
            y_final = yy[:, -1] if yy.ndim == 5 else yy          # >>> CHANGED

            pred_r = pred_primary.permute(0, 2, 3, 1).unsqueeze(-2)
            y_final_r = y_final.permute(0, 2, 3, 1).unsqueeze(-2)
