...
# ──────────────────────────────────────────────────────────────────────
# metric_func  (unchanged logic, but now copes with single-frame inputs)
# ──────────────────────────────────────────────────────────────────────
...
def metric_func(pred, target, if_mean=True,
    ...
    Expects   [B, X, Y(, Z), T, C]   but also accepts a single-frame
    prediction/target shaped [B, C, H, W].
    """
    pred, target = pred.to(device), target.to(device)

    # ── add dummy time dim if ViT returns one frame ───────────────────
    if pred.ndim == 4:                       # (B,C,H,W)
        pred = pred.permute(0, 2, 3, 1).unsqueeze(-2)   # B,H,W,1,C
        target = target.permute(0, 2, 3, 1).unsqueeze(-2)

    # everything below is **byte-for-byte** your original body
    #  (shortened here for brevity – copy it exactly)
    # -----------------------------------------------------------------
    # ……………………… ( keep entire original metric_func implementation ) …
    # -----------------------------------------------------------------
    idxs = target.size()
    if len(idxs) == 4:  # 1D
...
# ──────────────────────────────────────────────────────────────────────
# metrics()  –  new  "TransformerAux"  branch
# ──────────────────────────────────────────────────────────────────────
def metrics(
    val_loader, model,
    Lx, Ly, Lz,
        ...
        with torch.no_grad():
            ...
            for itot, (xx, yy, xx_aux, yy_aux, grid, grid_aux) in enumerate(val_loader):
                ...
                if len(idx) == 5:  # 2D
                    ...
                    grid_aux = grid_aux.unsqueeze(
                        1).expand(-1, num_aux, -1, -1, -1)
                ...
                # for _t in range(initial_step, yy.shape[-2]):
                    ...
                    # inp_aux = xx_aux.reshape(inp_shape)
                pred, _ = model(xx, grid, xx_aux, grid_aux)
                # pred = torch.cat((pred, im_primary), -2)
                # xx = torch.cat((xx[..., 1:, :], im_primary), dim=-2)  # noqa: PLW2901
    ...
    # ── final aggregation & logging (identical) ───────────────────────
